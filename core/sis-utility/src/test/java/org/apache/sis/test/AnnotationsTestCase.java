...
public abstract strictfp class AnnotationsTestCase extends TestCase {
	...
	private String assertExpectedNamespace(String namespace, final Class<?> impl, final UML uml) {
		...
		if (uml != null) {
			// Need to convert annotated namespace to ISO 19139 because implementation is now in ISO 19115-3.
			FilterVersion filter = FilterVersion.ISO19139;
			String newNamespace = filter.toView.get(namespace);
			// Special case for GMI namespace.
			XmlRootElement rootElement = impl.getAnnotation(XmlRootElement.class);
			if(rootElement != null && rootElement.name().startsWith("MI_")) {
				newNamespace = LegacyNamespaces.GMI;
			}
			// Actually perform the test.
			try {
				assertEquals("Wrong namespace for the ISO specification.",
						getExpectedNamespace(impl, uml.specification()), newNamespace);
				// In case the original test fails, see if the ISO 19115-3 namespace is specified in the UML.
	...
	public void testImplementationAnnotations() {
		for (final Class<?> type : types) {
			...
			// Adjust for changes in ISO 19115-3 standard.
			String name = root.name();
			if(name.startsWith("MD_Scope") && !name.startsWith("MD_ScopeDescription")) {
				name = name.replaceAll("MD_", "DQ_");
			}
			// Perform the actual check now.
			if (uml != null) {
				assertEquals("Wrong @XmlRootElement.name().", getExpectedXmlElementName(null, uml), name);
			}
			/*
			...
			// Adjust for changes in ISO 19115-3 standard.
			String newName = xmlType.name();
			if(newName.startsWith("MD_Scope") && !name.startsWith("MD_ScopeDescription")) {
				newName = newName.replaceAll("MD_", "DQ_");
			}
			assertEquals("Wrong @XmlType.name().", expected, newName);
		}
		done();
	}
