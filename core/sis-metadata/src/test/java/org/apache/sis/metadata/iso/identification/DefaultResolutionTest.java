...
public final strictfp class DefaultResolutionTest extends TestCase {
	...
	public void testXML19139() throws JAXBException {
		 ...
		 * an unknown reason.
		 */
		assertXmlEquals(testXmlXML(), xml, "xmlns:*");
		/*
	...
	public void testXML191153() throws JAXBException {
		final DefaultResolution resolution = new DefaultResolution();
		resolution.setDistance(1000.0);
		final String xml = XML.marshal(resolution, Namespaces.ISO_19115_3);
		assertTrue("<gmd:distance> element is missing. If this test fails randomly, "
				+ "see DefaultResolutionTest.testXML() javadoc for more information", xml.contains("distance"));
		/*
		 * Following test is done as a matter of principle, but should not be a problem.
		 ...
		 * an unknown reason.
		 */
		assertXmlEquals(ISOTestUtils.from19139(testXmlXML()), xml, "xmlns:*");
		/*
		 * Unmarshal the element back to a Java object, as a safety.
		 * Should not be a problem neither.
		 */
		assertEquals(resolution, XML.unmarshal(xml, Namespaces.ISO_19115_3));
	}

	private static String testXmlXML() {
		return "<gmd:MD_Resolution xmlns:gmd=\"" + Namespaces.GMD + '"' +
				" xmlns:gco=\"" + LegacyNamespaces.GCO + "\">\n" +
				"  <gmd:distance>\n" +
				"    <gco:Distance uom=\"" + Schemas.METADATA_ROOT_19139 + Schemas.UOM_PATH + "#xpointer(//*[@gml:id='m'])\">1000.0</gco:Distance>\n" +
				"  </gmd:distance>\n" +
				"</gmd:MD_Resolution>";
	}
}
